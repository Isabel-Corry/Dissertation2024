...
add_summary_stats <- function(x, y, color, main_title, x_label, y_label) {
  model <- lm(y ~ x)
  plot(x, y, pch = 19, col = color, main = main_title, xlab = x_label, ylab = y_label, xlim = c(0, 150), ylim = c(0, 150))
  abline(model, col = "red")
  mean_x <- mean(x)
  mean_y <- mean(y)
  abline(h = mean_y, col = "yellow", lty = 2)
  abline(v = mean_x, col = "green", lty = 2)
  text(mean_x, 150, labels = paste("Slope:", round(coef(model)[2], 2)), pos = 4, col = "grey")
  text(mean_x, 145, labels = paste("Intercept:", round(coef(model)[1], 2)), pos = 4, col = "grey")
}
