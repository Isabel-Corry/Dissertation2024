...
ESA <- rast("ESA_Resample_Mean.tif")
ICESat2 <- rast("ICESat2_Resample_Mean.tif")
ALS <- rast("ALS_Biomass_Extract_Resampled.tif")
GEDI_ci <- calculate_90_ci(GEDI)
ESA_ci <- calculate_90_ci(ESA)
ICESat2_ci <- calculate_90_ci(ICESat2)
ALS_ci <- calculate_90_ci(ALS)
...
print(paste("ALS 90% CI:", ALS_ci[1], "-", ALS_ci[2]))
ci_df <- data.frame(
  +     Dataset = c("GEDI", "ESA", "ICESat2", "ALS"), 
  +     Lower_Bound = c(GEDI_ci[1], ESA_ci[1], ICESat2_ci[1], ALS_ci[1]),
  +     upper_Bound = c(GEDI_ci[2], ESA_ci[2], ICESat2_ci[2], ALS_ci[2])
  +     )
print(ci_df)
